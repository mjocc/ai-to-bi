...
def load_model(model_dir=None):
    ...
    # Prepare the model for fast repeated inference:
    # 1) Create the internal predict function (avoids slow first-call trace)
    try:
        model.make_predict_function()
    except Exception:
        # make_predict_function may not be necessary in some TF builds
        pass

    # 2) Warm up the model by running a single dummy prediction to initialize
    # device kernels and caches (important on GPUs/Metal where shader compilation
    # can make the first real run much slower).
    try:
        dummy_input = np.zeros((1, CONFIG.get('img_height', 224), CONFIG.get('img_width', 224), 3), dtype=np.float32)
        _ = model.predict(dummy_input, verbose=0)
    except Exception:
        # If warmup fails for any reason, ignore and return the model anyway
        pass
